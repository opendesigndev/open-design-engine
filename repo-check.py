...
if len(sys.argv) >= 2:
    if not re.match(r'^[A-Z`]', sys.argv[1]):
        print('Error: Wrong format of commit message')
        ok = False

reGeneratedApiBindings = re.compile('\sgenerated by[\s\'\"`\<]*generate-api-bindings\.py[\s\'\"`\>]', re.IGNORECASE)
generatedApiBindingsPaths = []
...
for path in glob.iglob(root+'/**/*', recursive=True):
    ...
    if isTextFile(relPath) and not ignoredPath(relPath):
        ...
        if reGeneratedApiBindings.search(text):
            generatedApiBindingsPaths.append(path)

# Check outputs of generate-api-bindings.py
for path in generatedApiBindingsPaths:
    os.replace(path, path+'.orig_backup')
os.system('python "'+os.path.join(root, 'generate-api-bindings.py')+'"')
for path in generatedApiBindingsPaths:
    with open(path+'.orig_backup', 'r') as f:
        a = f.read()
    with open(path, 'r') as f:
        b = f.read()
    if a != b:
        print('Error: Generated file '+os.path.relpath(path, root)+' outdated or manually modified', file=sys.stderr)
        ok = False
    os.replace(path+'.orig_backup', path)
