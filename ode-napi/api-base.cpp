#include "addon.h"
#include <ode/api-base.h>
#include "napi-wrap.h"
Napi::Value bind_ode_makeString(const Napi::CallbackInfo &info);

Napi::Object init_api_base(Napi::Env env, Napi::Object exports) {
    exports.Set("makeString", Napi::Function::New<bind_ode_makeString>(env, "makeString"));
    return exports;
}

template<>
bool Autobind<int>::read_into(const Napi::Value &value, int &parsed) {
    parsed = (int32_t) value.As<Napi::Number>();
    return true;
}
template<>
Napi::Value Autobind<int>::serialize(Napi::Env env, const int &value) {
    return Napi::Number::New(env, value);
}


template<>
bool Autobind<unsigned long>::read_into(const Napi::Value &value, unsigned long &parsed) {
    parsed = (int64_t) value.As<Napi::Number>();
    return true;
}
template<>
Napi::Value Autobind<unsigned long>::serialize(Napi::Env env, const unsigned long &value) {
    auto sgn = static_cast<int64_t>(value);
    if (static_cast<unsigned long>(sgn) != value || sgn < 0) {
        Napi::Error::New(env, "Too big to convert to js number").ThrowAsJavaScriptException();
        return Napi::Value();
    }
    return Napi::Number::New(env, (uint32_t) value);
}


template<>
bool Autobind<ODE_Vector2>::read_into(const Napi::Value &value, ODE_Vector2 &parsed) {
    Napi::Array arr = value.As<Napi::Array>();
    parsed.x = (ODE_Scalar) arr.Get(uint32_t(0)).Unwrap().As<Napi::Number>();
    parsed.y = (ODE_Scalar) arr.Get(uint32_t(1)).Unwrap().As<Napi::Number>();
    return true;
}
template<>
Napi::Value Autobind<ODE_Vector2>::serialize(Napi::Env env, const ODE_Vector2 &parsed) {
    Napi::Error::New(env, "Not implemented: Autobind<ODE_Vector2>::serialize").ThrowAsJavaScriptException();
    return Napi::Value();
}


template<>
bool Autobind<ODE_Rectangle>::read_into(const Napi::Value &value, ODE_Rectangle &parsed) {
    Napi::Error::New(value.Env(), "Not implemented: Autobind<ODE_Rectangle>::read_into").ThrowAsJavaScriptException();
    return false;
}
template<>
Napi::Value Autobind<ODE_Rectangle>::serialize(Napi::Env env, const ODE_Rectangle &parsed) {
    Napi::Error::New(env, "Not implemented: Autobind<ODE_Rectangle>::serialize").ThrowAsJavaScriptException();
    return Napi::Value();
}


template<>
bool Autobind<ODE_ConstDataPtr>::read_into(const Napi::Value &value, ODE_ConstDataPtr &parsed) {
    Napi::Error::New(value.Env(), "Not implemented: Autobind<ODE_ConstDataPtr>::read_into").ThrowAsJavaScriptException();
}
template<>
Napi::Value Autobind<ODE_ConstDataPtr>::serialize(Napi::Env env, const ODE_ConstDataPtr &parsed) {
    Napi::Error::New(env, "Not implemented: Autobind<ODE_ConstDataPtr>::serialize").ThrowAsJavaScriptException();
    return Napi::Value();
}



template<>
bool Autobind<ODE_VarDataPtr>::read_into(const Napi::Value &value, ODE_VarDataPtr &parsed) {
    Napi::Error::New(value.Env(), "Not implemented: Autobind<ODE_VarDataPtr>::read_into").ThrowAsJavaScriptException();
    return false;
}
template<>
Napi::Value Autobind<ODE_VarDataPtr>::serialize(Napi::Env env, const ODE_VarDataPtr &parsed) {
    Napi::Error::New(env, "Not implemented: Autobind<ODE_VarDataPtr>::serialize").ThrowAsJavaScriptException();
    return Napi::Value();
}


template<>
bool Autobind<ODE_Scalar>::read_into(const Napi::Value &value, ODE_Scalar &parsed) {
    parsed = (ODE_Scalar) value.As<Napi::Number>();
    return true;
}
template<>
Napi::Value Autobind<ODE_Scalar>::serialize(Napi::Env env, const ODE_Scalar &parsed) {
    Napi::Error::New(env, "Not implemented: Autobind<ODE_Scalar>::serialize").ThrowAsJavaScriptException();
    return Napi::Value();
}


Napi::Value bind_ode_makeString(const Napi::CallbackInfo &info) {
    auto env = info.Env();
    auto string = ode_makeString(info[0].As<Napi::String>());
    auto obj = Napi::Object::New(env);
    obj.Set("data", Napi::Number::New(env, (uintptr_t) string.data));
    obj.Set("length", Napi::Number::New(env, string.length));
    return obj;
}

