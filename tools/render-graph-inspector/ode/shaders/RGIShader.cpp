...
RGIShader::SharedVertexShader::SharedVertexShader(const char *vsLabel) :
    VertexShader(vsLabel) {
}

bool RGIShader::SharedVertexShader::initialize() {
...
bool RGIShader::initialize(const SharedVertexShader &sharedVertexShader, const char *fsLabel) {
...
void RGIShader::bind(const ScaledBounds& srcBounds, const ScaledBounds& dstBounds) {
    shader_.bind();
